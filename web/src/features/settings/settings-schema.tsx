...
const fieldLabels: Record<string, string> = {
  ...
  stream_keepalive_seconds: "流式保活间隔",
  stream_idle_timeout_seconds: "流式空闲保护时长",
  stream_max_duration_seconds: "流式最长持续时长",
  provider_circuit_failure_threshold: "Provider 熔断失败阈值",
  provider_circuit_open_seconds: "Provider 熔断初始打开时长",
  provider_circuit_max_open_seconds: "Provider 熔断最大打开时长",
  provider_circuit_half_open_max: "Provider 半开探针并发",
...
export function buildSettingGroups(response: AdminSettingsResponse) {
  ...
  return {
    ...
    advancedFields: [
      ...
      mkField("stream_keepalive_seconds", "流式保活间隔（秒）", "SSE 流空闲时发送注释保活，避免中间层误判连接闲置。", () => <InputNumber min={1} step={1} className="full-width" />),
      mkField("stream_idle_timeout_seconds", "流式空闲保护（秒）", "流式响应超过该时长没有收到上游数据时终止并记录诊断。", () => <InputNumber min={1} step={1} className="full-width" />),
      mkField("stream_max_duration_seconds", "流式最长持续（秒）", "单个流式响应超过该总时长时终止，防止异常长连接无限占用资源。", () => <InputNumber min={1} step={1} className="full-width" />),
      mkField("provider_circuit_failure_threshold", "Provider 熔断失败阈值", "当前 active provider 连续发生 provider 级失败达到该次数后打开熔断。", () => <InputNumber min={1} step={1} className="full-width" />),
      mkField("provider_circuit_open_seconds", "Provider 熔断初始打开时长（秒）", "provider 熔断首次打开后等待多久再进入半开探测。", () => <InputNumber min={1} step={1} className="full-width" />),
      mkField("provider_circuit_max_open_seconds", "Provider 熔断最大打开时长（秒）", "provider 连续恢复失败时，打开时长退避增长到该上限。", () => <InputNumber min={1} step={1} className="full-width" />),
      mkField("provider_circuit_half_open_max", "Provider 半开探针并发", "provider 熔断半开阶段允许同时放行的探针请求数量。", () => <InputNumber min={1} step={1} className="full-width" />),
    ] as SettingFieldMeta[],
    networkFields: [
